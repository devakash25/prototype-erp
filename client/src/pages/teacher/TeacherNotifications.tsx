...
export function TeacherNotifications() {
  const { data, loading, error, refetch } = useApi<any[]>('/teacher/notifications')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Notifications</h1><p className="text-gray-500 text-sm">Your notifications</p></div>
      ...
      <div className="space-y-3">
          ...
          <div key={n.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1"><h3 className="font-semibold text-gray-900">{n.title}</h3>
        ...
        {notifications.length === 0 && <div className="flex flex-col items-center justify-center py-16 text-gray-500 gap-2"><AlertCircle className="w-8 h-8" /><p>No notifications</p></div>}
      </div>
    </div>
  )
}
