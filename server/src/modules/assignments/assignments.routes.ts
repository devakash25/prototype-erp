...
router.get('/teacher/subjects', authorize('TEACHER'), wrap(async (req: Request, res: Response) => {
  const user = (req as any).user;
  const data = await assignmentService.getTeacherSubjects(user.userId);
  res.json({ success: true, data });
}));

router.get('/teacher/subjects/:subjectId/assignments', authorize('TEACHER'), wrap(async (req: Request, res: Response) => {
  const user = (req as any).user;
  const data = await assignmentService.getSubjectAssignments(user.userId, req.params.subjectId as string);
  res.json({ success: true, data });
}));

router.post('/teacher/assignments', authorize('TEACHER'), validate(createAssignmentSchema), wrap(async (req: Request, res: Response) => {
  const user = (req as any).user;
...
router.get('/teacher/assignments/:assignmentId/submissions', authorize('TEACHER'), wrap(async (req: Request, res: Response) => {
  const user = (req as any).user;
  const data = await assignmentService.getAssignmentSubmissions(user.userId, req.params.assignmentId as string);
  res.json({ success: true, data });
}));

router.post('/teacher/grade', authorize('TEACHER'), validate(gradeSubmissionSchema), wrap(async (req: Request, res: Response) => {
  const user = (req as any).user;
